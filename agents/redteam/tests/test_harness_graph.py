"""Tests for run_session_via_graph() / build_graph() — the LangGraph
orchestration of the same loop test_harness.py exercises directly. Same
mocked-router, InMemoryChronoDAG pattern as test_harness.py.
"""
...
from agents.redteam.harness import run_session_via_graph
...
def test_graph_session_stops_at_step_cap_without_commit(
    shared_dag: InMemoryChronoDAG, router_config: RedTeamConfig, tmp_path: Path
) -> None:
    sim_config = SimConfig(seed=42, db_url="postgresql://mock:5432")
    checkpoint = _make_checkpoint(sim_config)
    ...
    with patch.object(router, "completion", side_effect=scripted):
        result = run_session_via_graph(
            ...
            session_id="graph-session-2", router=router,
            identity_file=tmp_path / ".persona_identity.json",
        )
