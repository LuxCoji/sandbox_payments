...
Additionally, there are three outer layers:
...
- `risk/`: fraud detection, two rails. **Injected, never imported** — `sim.core.interfaces` defines a `RiskScorer` protocol and `sim.main._build_risk` is the only place in `sim/` allowed to construct an implementation, enforced by a sixth import-linter contract. See [docs/fraud_detection.md](docs/fraud_detection.md).
...
- **`enable_risk` defaults to `False`, and `scripts/red_team_run.py` inverts that.** The global default keeps the engine byte-identical to what every replay/determinism/state-hash test expects — turning risk on changes which payments emit `PaymentDeclined`, which changes the event stream and the state hash. But the red-team path called `build_simulation` and read that default, so every attack session ran against a simulator with no controls, and every "nothing flagged it" finding was true and meaningless. If you add another entry point that runs an adversary, arm it explicitly.
- **`risk/card/encoding.py` is the only place FinSim fields map onto the model schema**, imported by both training and serving. Building that vector twice is the classic silent model failure: no exception, no obviously wrong number, just worse predictions that read as a weak model. If you add a feature, append to `DYNAMIC_NUM` — never insert, because the list defines tensor column order and a model trained on one order scores nonsense under another.
- **The supervised loss reads `label_mask`, not `~pad_mask`.** `to_sequences` emits one sequence per transaction, so a fraudulent row reappears as a non-final position in up to 31 later sequences, where it sits at a placeholder zero. A loss over every real position therefore saw that row once labelled fraud and twenty-nine times labelled genuine. Serving reads one position — the last — so the loss must too.
- **A container that defines `__len__` is falsy when empty**, so `history or AccountHistory()` silently discards the caller's object. That bit `risk/engine.py` for real: the engine and its card scorer ended up with separate histories, the scorer's sequences never reached the traffic recorder, and the collected training set came out empty with no error anywhere. Use `is None`.
